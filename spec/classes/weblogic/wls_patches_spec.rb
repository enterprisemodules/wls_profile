  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context "patch window defined" do
        ...
        context "Managed servers are running" do
          let(:test_facts) {{
            'wls_install_homes' => {
              'running_domains' => {'/opt/oracle/middleware12' => ['dom01']},
              'opatch_version' => {'/opt/oracle/middleware12' => '12.2.1.4'},
          ...
          context "on non-adminserver" do
            ...
            it { is_expected.to contain_wls_profile__weblogic__private__stop_managed_servers('dom01')
              .with('schedule_name' => 'wls_patch_window')
            }
            
            it { is_expected.to contain_wls_profile__weblogic__private__start_managed_servers('dom01')
              .with('schedule_name' => 'wls_patch_window')
            }
            ...
            it { is_expected.to contain_wls_opatch('/opt/oracle/middleware12:32253037')
              ...
              .that_comes_before('Wls_profile::Weblogic::Private::Start_managed_servers[dom01]')
              .that_requires('Wls_profile::Weblogic::Private::Stop_managed_servers[dom01]')
            }
          ...
          context "On Adminserver node" do
            next if os =~ /aix/
            let(:params) {{
              'version'             => 12214,
              'level'               => 'JAN2021RU',
              'patch_window'        => '00:00 - 23:59',
              'adminserver_address' => os_facts[:ipaddress]
            }}
    
            it { is_expected.to contain_wls_install__control('start_adminserver_dom01_after_patches')
              .with('action'                      => 'start')
              .with('domain_name'                 => 'dom01')
              .with('schedule'                    => 'wls_patch_window')
              .that_requires('Sleep[Wait for nodemanager on domain dom01]')
              .that_comes_before('Wls_exec[dom01/@/tmp/start_managed_servers_for_dom01.py]')
            }
